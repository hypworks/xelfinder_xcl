...
class Xelfinder_Utils
{
    ...
    **/
    ...
    {
        $action = isset($action) ? $action : Xelfinder_AuthType::VIEW;
        ...
        if($role=='cat'){
            ...
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), self::getActor($dirname, Xelfinder_AuthType::VIEW), Legacy_Utils::getUid(), $categoryId);
        }
        elseif($role=='group'){
            $delegateName = 'Legacy_Group.'.$accessController->dirname().'.GetGroupIdListByAction';
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), $dirname, 'page', Xelfinder_AuthType::VIEW);
        }
    ...
    **/
     ...
     * 
     * @return  Xelfinder_AbstractAccessController
    **/
	public static function getAccessControllerObject(/*** string ***/ $dirname, /*** string ***/ $dataname)
	{
		...
		if(! $module){
			require_once XELFINDER_TRUST_PATH . '/class/NoneAccessController.class.php';
			$accessController = new Xelfinder_NoneAccessController($server, $dirname, $dataname);
		...
		switch($role){
		case 'cat':
			require_once XELFINDER_TRUST_PATH . '/class/CatAccessController.class.php';
			$accessController = new Xelfinder_CatAccessController($server, $dirname, $dataname);
			break;
		case 'group':
			require_once XELFINDER_TRUST_PATH . '/class/GroupAccessController.class.php';
			$accessController = new Xelfinder_GroupAccessController($server, $dirname, $dataname);
			break;
		case 'none':
		default:
			require_once XELFINDER_TRUST_PATH . '/class/NoneAccessController.class.php';
    ...
    **/
    public static function getActor(/*** string ***/ $dirname, /*** string ***/ $action)
    {
        ...
        switch($action){
            case Xelfinder_AuthType::VIEW:
                return trim($authType[0]);
                break;
            case Xelfinder_AuthType::POST:
                return trim($authType[1]);
                break;
            case Xelfinder_AuthType::MANAGE:
