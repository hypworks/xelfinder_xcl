...
require_once XELFINDER_TRUST_PATH . '/class/AbstractAction.class.php';

define('XELFINDER_FRAME_PERFORM_SUCCESS', 1);
define('XELFINDER_FRAME_PERFORM_FAIL', 2);
define('XELFINDER_FRAME_INIT_SUCCESS', 3);

define('XELFINDER_FRAME_VIEW_NONE','none');
define('XELFINDER_FRAME_VIEW_SUCCESS','success');
define('XELFINDER_FRAME_VIEW_ERROR','error');
define('XELFINDER_FRAME_VIEW_INDEX','index');
define('XELFINDER_FRAME_VIEW_INPUT','input');
define('XELFINDER_FRAME_VIEW_PREVIEW','preview');
define('XELFINDER_FRAME_VIEW_CANCEL','cancel');

/**
 * Xelfinder_Module
**/
class Xelfinder_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Xelfinder_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Xelfinder_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		XELFINDER_FRAME_VIEW_NONE,
		XELFINDER_FRAME_VIEW_SUCCESS,
		XELFINDER_FRAME_VIEW_ERROR,
		XELFINDER_FRAME_VIEW_INDEX,
		XELFINDER_FRAME_VIEW_INPUT,
		XELFINDER_FRAME_VIEW_PREVIEW,
		XELFINDER_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.xelfinder.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'File';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('XELFINDER_ADMIN_RENDER_REGISTED'))
		{
			define('XELFINDER_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Xelfinder_AdminRenderSystem' => 'Xelfinder_AdminRenderSystem'
					),
					'Xelfinder_AdminRenderSystem' => array(
						'root' => XELFINDER_TRUST_PATH,
						'path' => '/admin/class/XelfinderAdminRenderSystem.class.php',
						'class' => 'Xelfinder_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Xelfinder_AdminRenderSystem';
	}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = XELFINDER_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Xelfinder_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Xelfinder_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.xelfinder.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.xelfinder.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.xelfinder.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.xelfinder.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.xelfinder.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.xelfinder.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
		...
		**/
		...
		$root =& XCube_Root::getSingleton();
		$root->mController->executeRedirect(XOOPS_URL, 1, _MD_XELFINDER_ERROR_PERMISSION);
	}
}
